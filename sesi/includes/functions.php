...
function deleteSubject($pdo, $subject_id) {
    try {
        ...
        $pdo->rollBack();
        return "Erro ao excluir o assunto: " . $e->getMessage();
    }
}
...
function deleteClassroom($pdo, $classroom_id) {
    try {
        ...
        $pdo->rollBack();
        return "Erro ao deletar a sala de aula: " . $e->getMessage();
    }
}
...
function deleteGrade($pdo, $grade_id, $teacher_id) {
    ...
    if ($stmt->rowCount() === 0) {
        return "Nota não encontrada ou você não tem permissão para excluí-la!";
    }
...
function removeStudentFromClassroom($pdo, $classroom_id, $student_id) {
    try {
        ...
        $pdo->rollBack();
        return "Erro ao remover estudante: " . $e->getMessage();
    }
}
